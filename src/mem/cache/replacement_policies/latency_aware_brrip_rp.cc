...
#include "debug/CacheRepl.hh" // Standard flag for replacement policies
...
LatencyAwareBRRIP::getVictim(const ReplacementCandidates& candidates) const
{
    ...
    // Since we are only using this for L2 Caches, we assume MachineType::L2Cache
    ...
    for (const auto& candidate : candidates) {
        ...
        if (ruby_entry) {
            ...
            // We assume that if the line is in THIS cache, it belongs to THIS NodeID.
